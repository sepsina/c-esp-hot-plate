...
static void getID(idCmd_t *idCmd);
...
int main(void) {
    ...
    //NVIC_SetPriority(RTC_Alarm_IRQn, 11);
...
static void rtcStart(uint16_t period) {
    ...
    rtc.cr_l.b.cnf = 0; // 0: Exit configuration mode
    REG_Write(RTC__CRL, rtc.cr_l.all);
}
*/
/****************************************************************************
    ...
    switch(msgType) {
        case MSG_STM_REQ: {
            ...
            switch(cmd->cmdID){
                ...
                case STM_GET_ID: {
                    idCmd_t idCmd;
                    idCmd.ip = req->clientIP;
                    idCmd.port = req->clientPort;
                    idCmd.pktFunc = cmd->pktFunc;
                    idCmd.cmdID = cmd->cmdID;
...
static void getID(idCmd_t *idCmd) {
    ...
    txPkt->clientIP = idCmd->ip;
    txPkt->clientPort = idCmd->port;
    txPkt->len = sizeof(idRsp_t);

    rsp = (idRsp_t *)&txPkt->data[0];
    rsp->pktFunc = idCmd->pktFunc;
    rsp->cmdID = idCmd->cmdID;
    rsp->id = HOT_PLATE_ID;

    slWriteMsg(MSG_STM_REQ,
               sizeof(txPkt_t) + sizeof(idRsp_t),
               &slTxBuf[0]);
}
